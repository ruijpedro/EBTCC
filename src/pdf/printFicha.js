    ...
    <div class="page photos-page">
      ...
      <div class="photo-grid">
        ${(data.photos || []).map((p,i) => `
          <div class="photo-box">
            <img src="${p.data}">
            <p><b>Foto ${i+1}</b>${p.caption ? " — " + escapeHtml(p.caption) : ""}</p>
          </div>`).join("")}
      </div>
    </div>` : "";
...
<style>
  ...
  .photos-page{padding-left:0}.photos-page h2{font-size:18px;margin:0 0 8mm}
  .photo-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8mm}
  .photo-box{border:1px solid #333;padding:3mm;break-inside:avoid}
  .photo-box img{width:100%;height:48mm;object-fit:cover;display:block}
  .photo-box p{font-size:10px;margin:2mm 0 0}
</style>
</head>
<body>
  <div class="page">
  ...
  ${photosHtml}
  <div class="footer">
    <span>Ficha de Inspeção - Inspeções</span>
